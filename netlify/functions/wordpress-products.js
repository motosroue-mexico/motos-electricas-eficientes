...
function getIdFromAcfValue(v){ if (v==null) return null;
  if (typeof v==='number' && Number.isFinite(v)) return v;
  if (typeof v==='string'){ const s=v.trim(); if (/^\d+$/.test(s)) return Number(s); try{ return getIdFromAcfValue(JSON.parse(s)); }catch{} }
...
async function fetchAllProducts(opts){
  let page=1; const perPage=opts.perPage || PER_PAGE_DEFAULT; const all=[];
  const first = await fetchProductsPage({ ...opts, page, perPage });
  all.push(...first.data); const totalPages = first.totalPages;
  while (++page <= totalPages) { const { data } = await fetchProductsPage({ ...opts, page, perPage }); all.push(...data); }
  return all;
}

async function fetchMediaMap(ids){
  const unique=[...new Set(ids.filter(Boolean))]; const map=new Map();
...
function shapeProduct(p, mediaMap){
  ...
  return {
    id:p.id, name:p.name, status:p.status, type:p.type, sku:p.sku,
    price:p.price, regular_price:p.regular_price, sale_price:p.sale_price,
    stock_status:p.stock_status, permalink:p.permalink, image:p.images?.[0]?.src || null,
    acf
  };
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const ids=[]; for (const p of products){
      const meta=Array.isArray(p.meta_data)?p.meta_data:[]; const e=meta.find(m=>m && m.key==='imagen-landing');
      const id=getIdFromAcfValue(e?.value); if (id) ids.push(id);
    }
    const mediaMap = await fetchMediaMap(ids);
    const shaped   = products.map(p => shapeProduct(p, mediaMap));

    return { statusCode:200, headers:{ 'Content-Type':'application/json; charset=utf-8','Access-Control-Allow-Origin':'*' }, body: JSON.stringify(shaped) };
  } catch (err) {
    return { statusCode:500, headers:{ 'Content-Type':'application/json; charset=utf-8','Access-Control-Allow-Origin':'*' }, body: JSON.stringify({ error: err.message }) };
  }
};
